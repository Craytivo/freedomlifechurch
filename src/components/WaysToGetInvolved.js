...
const Card = ({ item, spanClass = '' }) => (
  <div className={`relative rounded-xl overflow-hidden shadow-sm border border-neutral-200 bg-white group ${spanClass}`}>
    {/* Soft background mark */}
    <div className={`absolute -right-10 -top-10 w-48 h-48 rounded-full ${item.bgMark}`} aria-hidden="true" />
    <div className="relative p-5 md:p-6 lg:p-7 flex flex-col h-[220px] md:h-[240px]">
      {/* Icon */}
      ...
      <div className="mt-auto pt-4">
        <Link
          ...
          className="inline-flex items-center text-sm font-semibold text-neutral-900 hover:text-neutral-700"
        >
          Learn more
          <svg className="w-4 h-4 ml-1" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path strokeLinecap="round" strokeLinejoin="round" d="M9 5l7 7-7 7"/></svg>
        </Link>
      </div>
    </div>
  </div>
    ...
    <section className="bg-neutral-50 py-12 md:py-16">
      <div className="mx-auto px-4 sm:px-6 lg:px-8" style={{ maxWidth: '88rem' }}>
        <div className="text-center max-w-3xl mx-auto">
          <h2 className="font-heading text-2xl md:text-3xl font-extrabold text-neutral-900">A place for you and your family</h2>
          <p className="mt-2 text-neutral-600">Join small groups, get involved in outreach, and discover ways for your whole family to grow in their faith.</p>
        </div>
