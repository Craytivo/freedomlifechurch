...
const SermonLibraryPreview = () => {
  ...
  return (
    <>
    <section id="sermon-library" className="relative py-12 sm:py-16 md:py-20 lg:py-24 bg-neutral-50/40 content-visibility-auto">
      ...
      <div className="mx-auto px-3 sm:px-4 lg:px-8" style={{ maxWidth: '82rem' }}>
        <div className="flex flex-col md:flex-row md:items-end md:justify-between gap-4 sm:gap-6 mb-8 sm:mb-10">
          <div>
            ...
            <h2 className="mt-2 sm:mt-3 font-heading text-2xl sm:text-3xl md:text-4xl font-bold tracking-tight text-primary-900 mb-2 sm:mb-3">Recent Messages</h2>
            <p className="text-neutral-600 text-sm sm:text-base md:text-lg max-w-xl leading-relaxed">Catch up on the latest teaching and explore past messages to keep growing mid‑week.</p>
          </div>
          ...
          <div className="lg:col-span-6">
            <div className="group relative rounded-2xl shadow-sm border border-neutral-200 overflow-hidden bg-white">
              ...
              <div className="p-5 sm:p-6 md:p-7">
                <div className="flex items-center gap-2 text-[10px] sm:text-[11px] font-semibold tracking-wide uppercase text-flc-600 mb-2">
                  <span>Recent</span>
                ...
                <h3 className="font-heading text-lg sm:text-xl md:text-2xl font-bold tracking-tight text-primary-900 mb-2 sm:mb-3">
                  {featured.title}
                </h3>
                <p className="text-neutral-600 text-xs sm:text-sm md:text-base leading-relaxed mb-4 sm:mb-5 max-w-prose line-clamp-4">{featured.description}</p>
                <div className="flex flex-col sm:flex-row sm:flex-wrap gap-2 sm:gap-3">
                  <button onClick={() => setModalVideo(featured.id)} className="inline-flex items-center justify-center gap-2 px-4 sm:px-5 py-2 sm:py-2.5 rounded-md bg-flc-500 hover:bg-flc-600 text-white text-xs sm:text-sm font-semibold shadow-sm focus:outline-none focus:ring-2 focus:ring-flc-500/40">
                    Watch Now
                    <svg className="w-3.5 h-3.5 sm:w-4 sm:h-4" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" d="M9 5l7 7-7 7"/></svg>
                  </button>
                  <a href="#sermons" className="inline-flex items-center justify-center gap-2 px-4 sm:px-5 py-2 sm:py-2.5 rounded-md border border-neutral-300 text-neutral-700 hover:border-flc-500 hover:text-flc-600 text-xs sm:text-sm font-medium focus:outline-none focus:ring-2 focus:ring-flc-500/30">
                    Full Series
                  </a>
                </div>
              </div>
          ...
          <div className="lg:col-span-6 space-y-4 sm:space-y-6">
            {rest.map(s => (
              <a
                key={s.id}
                href="#"
                className="group flex gap-3 sm:gap-4 lg:gap-5 p-3 sm:p-4 md:p-5 rounded-xl border border-neutral-200 bg-white hover:border-flc-500/50 hover:shadow-sm transition-colors focus:outline-none focus:ring-2 focus:ring-flc-500/30"
                onClick={(e) => { e.preventDefault(); setModalVideo(s.id); }}
              >
                <div className="relative w-28 sm:w-32 lg:w-40 shrink-0 rounded-md overflow-hidden">
                  <div className="relative w-full" style={{ paddingBottom: '56.25%' }}>
                    <Image
